...
class PromptServer():
    def __init__(self, loop):
        ...
        async def websocket_handler(request):
            print("Received request to connect through websocket")
            ...
            if "prompt" in json_data:
                ...
                if valid[0]:
                    if "prompt_id" in json_data:
                        prompt_id = json_data["prompt_id"] # The prompt_id is generated by the global-dispatcher now
                    else:
    ...
    async def send_bytes(self, event, data, sid=None):
        if self.ws == None or self.ws.closed:
            logging.warning("WebSocket to local dispatcher is not open, cannot send message")
            return
